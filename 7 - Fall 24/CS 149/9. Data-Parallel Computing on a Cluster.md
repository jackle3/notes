...
![[Pasted image 20241104112508.png]]
...
![[Pasted image 20241104113128.png]]
* **When you read:**
...
![[Pasted image 20241104120151.png]]
...
![[Pasted image 20241104113809.png|600]]
### Reduce
...
![[Pasted image 20241104114519.png]]
...
![[Pasted image 20241104120833.png]]
...
![[Pasted image 20241104121105.png]]
...
![[Pasted image 20241104122035.png]]
...
![[Pasted image 20241104122356.png]]
...
![[Pasted image 20241104134122.png]]
...
![[Pasted image 20241104135510.png]]
### Examples
* In terms of length of RDDs: `lines` (an RDD) > `mobileViews` (an RDD) > `safariViews` (an RDD) >> `numViews` (an int)
![[Pasted image 20241104135302.png]]
* In this case we chain multiple transformations together to compute the final result.
![[Pasted image 20241104135402.png]]
...
![[Pasted image 20241104135639.png]]

## Implementing RDDs

### How to Store RDDs in Memory?
* **Materialization** ⟶ store the actual data in memory.
* If we treated RDDs as arrays (i.e. materializing it):
    * We would need more memory than the file size.
    * We would have ineffective use of the cache.

### Loop Fusion
* Recall the idea of loop fusion from [[6. Locality + Communication + Contention#Loop Fusion]] where we fuse independent operations from two loops into a single loop.
* Spark can do a similar thing by fusing RDD operations into streaming tasks.
	* Keep track of the lineage (sequence of transformations) needed to compute the RDD.
	* This allows Spark to pipeline operations and avoid materializing intermediate results.
	* Example: `map().filter().reduce()` can be fused into a single streaming task.
* Benefits of loop fusion in Spark:
	* Reduces memory usage by avoiding intermediate data storage
	* Improves cache efficiency by processing data in a streaming fashion
	* Reduces communication overhead between nodes
* Limitations:
	* Not all operations can be fused (e.g. operations requiring shuffling)
	* Complex dependencies may prevent fusion
![[Pasted image 20241104140613.png]]
